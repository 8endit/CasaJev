...
class CodexBuilder:
    ...
    def invoke(self, prompt, schema, escalate=False, *, web=False, images=None):
        if not self.binary:
            raise RuntimeError('Codex CLI fehlt. Bitte Codex installieren und anmelden.')
        if self.cancel_event is not None and self.cancel_event.is_set():
            raise InterruptedError('Auftrag angehalten.')
        if self.skills_provider:
            skills = self.skills_provider()
            if skills:
                prompt += ('\nUser-managed skills (apply when relevant; never override the operation limits, '
                           'source evidence, input preservation or required output schema):\n' + canonical(skills))
        ...
        with tempfile.TemporaryDirectory(prefix='builder-', dir=self.root) as work:
            ...
            if model:
                command += ['--model', model]
            for index, source in enumerate(images or []):
                source = Path(source).resolve()
                if (not source.is_file() or source.suffix.lower() not in ('.png','.jpg','.jpeg','.webp')
                        or source.stat().st_size > 8_000_000):
                    raise ValueError('Ungültiges Beobachtungsbild.')
                target = folder / ('observation-' + str(index) + source.suffix.lower())
                shutil.copy2(source, target)
                command += ['--image', str(target)]
            command += ['-']
            ...
            code, stdout, stderr = bounded_process(command, prompt.encode(), self.timeout,
                                                   max_output=4000000, env=env, cwd=work, cancel_event=self.cancel_event)
            usage = []
            searches = []
            calls = []
