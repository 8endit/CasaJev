...
class FaultRunner:
    ...
    def run(self,*args):
        if self.fault and not self.injected:
            self.injected+=1
            raise RuntimeError('Synthetic transient worker failure before execution; retry is safe')
        return self.runner.run(*args)


def evaluate(case,state,events,plan=None):
    decisions=[e['body']['answers'].get('action') for e in events if e['kind']=='jev_decision']
    reviews=[e['body']['choice'] for e in events if e['kind']=='decision_review']
    action=(reviews[-1] if reviews else (decisions[-1] if decisions else None))
    status=state['status']
